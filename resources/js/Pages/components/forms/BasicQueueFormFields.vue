                                        ...
                                        <div
                                            ...
                                            <FormElements>
                                                ...
                                                <SelectElement v-if="extendedFields" name="queue_announce_sound"
                                                    ...
                                                    placeholder="Select announcement" :strict="false"
                                                    :columns="{ sm: { container: 6 } }" />
...
const exitKeyOptions = Array.from({ length: 11 }, (_, i) => {
    const value = String(i);
    return { value, label: value === "0" ? "Disabled" : value };
});

const timeBaseScoreOptions = [
...
const defaultValues = computed(() => ({
    ...
    queue_tier_rule_wait_second: props.options?.item?.queue_tier_rule_wait_second ?? 30,
    queue_tier_rule_wait_multiply_level: props.options?.item?.queue_tier_rule_wait_multiply_level ?? "false",
    queue_tier_rule_no_agent_no_wait: props.options?.item?.queue_tier_rule_no_agent_no_wait ?? "false",
    queue_cc_exit_keys: props.options?.item?.queue_cc_exit_keys ?? null,
    queue_announce_sound: props.options?.item?.queue_announce_sound || "null",
    queue_announce_frequency: props.options?.item?.queue_announce_frequency ?? null,
    queue_announce_position: props.options?.item?.queue_announce_position ?? "false",
    queue_time_base_score: props.options?.item?.queue_time_base_score ?? "system",
    queue_time_base_score_sec: props.options?.item?.queue_time_base_score_sec ?? null,
    queue_discard_abandoned_after: props.options?.item?.queue_discard_abandoned_after ?? null,
    queue_email_address: props.options?.item?.queue_email_address ?? null,
    queue_abandoned_resume_allowed: props.options?.item?.queue_abandoned_resume_allowed ?? "false",
    queue_record_template: props.options?.item?.queue_record_template ?? "false",
...
const fetchAnnouncementSounds = async () => {
    const greetings = await fetchGreetings();

    return [
        { value: "null", label: "No announcement" },
        ...greetings.filter((item) => !["disabled", "0"].includes(String(item.value))),
    ];
};
...
const handleSuccess = (response) => {
    const messages = response.data.messages ?? { success: [response.data.message || "Saved."] };
