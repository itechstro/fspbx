    ...
    <MainLayout>
        <div class="m-3 space-y-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="text-lg font-semibold text-gray-900">Call History Analytics</h1>
                    <p class="text-sm text-gray-500">Summary stats and per-call summaries for call history.</p>
                </div>
                <a :href="routes.call_history_page"
            ...
            <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <div class="relative z-10 min-w-64">
                        <label class="block text-xs font-medium text-gray-500">Date range</label>
                    ...
                    <div class="min-w-36">
                        ...
                        <multiselect v-model="filterData.direction" :options="callDirections" :searchable="false"
                            ...
                            placeholder="All directions" class="mt-1" />
                    </div>
                    <div class="min-w-36">
                        <label class="block text-xs font-medium text-gray-500">Status</label>
                        <multiselect v-model="filterData.status" :options="statusOptions" :searchable="false"
                            :close-on-select="true" track-by="value" label="name" :show-labels="false"
                            placeholder="All statuses" class="mt-1" />
                    </div>
                    <div v-if="permissions.all_cdr_view" class="min-w-56">
                        <label class="block text-xs font-medium text-gray-500">Extension or queue</label>
                        <multiselect v-model="filterData.entity" :options="entities" deselect-label=""
                            select-group-label="" deselect-group-label="" select-label="" group-values="groupOptions"
                            group-label="groupLabel" :group-select="false" placeholder="All extensions"
                            track-by="value" label="label" class="mt-1" />
                    </div>
                    <div v-if="permissions.search_sentiment" class="min-w-36">
                        <label class="block text-xs font-medium text-gray-500">Sentiment</label>
                        <multiselect v-model="filterData.sentiment" :options="sentimentOptions" :searchable="false"
                            :close-on-select="true" track-by="value" label="name" :show-labels="false"
                            placeholder="All sentiment" class="mt-1" />
                    </div>
                ...
                <p v-if="hasActiveFilters" class="mt-1 text-sm text-gray-500">
                    Filters apply to the report, CSV export, executive summary, and emailed reports.
                </p>
            ...
            <template v-else-if="report">
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <AnalyticsDoughnutCard
                        ...
                        :breakdown="report.recording_status_breakdown"
                        :colors="recordingStatusColors"
                        empty-text="No recording data for this period."
                    />
                    <AnalyticsDoughnutCard
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <div class="rounded-lg bg-white p-5 ring-1 ring-gray-200">
                        <h2 class="mb-4 text-sm font-semibold text-gray-900">Call Direction</h2>
                        <div v-if="directionTotal === 0" class="py-10 text-center text-sm text-gray-500">
                        ...
                        <div v-else class="flex flex-col items-center gap-4">
                            ...
                            <div class="w-full space-y-1">
                                <div v-for="item in directionBreakdown" :key="item.direction"
                                    class="flex items-center justify-between text-xs">
                                    <div class="flex items-center gap-2">
                                        <span class="inline-block h-2.5 w-2.5 rounded-full"
                                            :style="{ backgroundColor: item.color }"></span>
                                    ...
                                    <div class="font-medium text-gray-900">
                                        {{ item.count }}
                                        <span class="ml-1 text-gray-400">{{ percent(item.count, directionTotal) }}%</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="rounded-lg bg-white p-5 ring-1 ring-gray-200">
                    <h2 class="mb-4 text-sm font-semibold text-gray-900">Calls</h2>
                    <div v-if="report.calls.length === 0" class="py-10 text-center text-sm text-gray-500">
                        No calls found for this period<span v-if="hasActiveFilters"> matching your filters</span>.
                    </div>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="call in report.calls" :key="call.xml_cdr_uuid" class="border-b border-gray-100 align-top">
                                    ...
                                    <td class="px-2 py-3 whitespace-nowrap">{{ formatDirection(call.direction) }}</td>
            ...
            <div v-if="permissions.schedule" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div class="rounded-lg bg-white p-5 ring-1 ring-gray-200">
                    ...
                    <p class="mb-4 text-sm text-gray-500">Send the current date range and filters to one or more recipients. Emails include a CSV attachment.</p>
                ...
                <div class="rounded-lg bg-white p-5 ring-1 ring-gray-200">
                    ...
                    <p class="mb-4 text-sm text-gray-500">
                        ...
                        The saved filters below apply to each scheduled report.
                    </p>
...
const directionBreakdown = computed(() => {
    return (report.value?.direction_breakdown ?? []).map((item, index) => ({
        direction: item.status,
        label: formatDirection(item.status),
        count: item.count ?? 0,
        color: statusColors[index % statusColors.length],
    }));
});

const directionTotal = computed(() => directionBreakdown.value.reduce((sum, item) => sum + item.count, 0));

const directionChartData = computed(() => ({
    labels: directionBreakdown.value.map((item) => item.label),
    datasets: [{
        data: directionBreakdown.value.map((item) => item.count),
        backgroundColor: directionBreakdown.value.map((item) => item.color),
        borderWidth: 0,
    }],
}));
...
const hasActiveFilters = computed(() => {
    return Boolean(
        ...
        || filterData.value.direction
        || filterData.value.status
        || filterData.value.entity
        || filterData.value.sentiment
    );
});
...
const buildFilterPayload = () => ({
    ...
    direction: filterData.value.direction?.value || null,
    status: filterData.value.status?.value || null,
    entity: filterData.value.entity
        ? { type: filterData.value.entity.type, value: filterData.value.entity.value }
        : null,
    sentiment: filterData.value.sentiment?.value || null,
});
...
const applySavedFilters = (filters = {}) => {
    if (filters.search) {
        filterData.value.search = filters.search;
    }

    if (filters.direction) {
        filterData.value.direction = callDirections.find((item) => item.value === filters.direction) || null;
    }

    if (filters.status) {
        filterData.value.status = statusOptions.find((item) => item.value === filters.status) || null;
    }

    if (filters.sentiment) {
        filterData.value.sentiment = sentimentOptions.find((item) => item.value === filters.sentiment) || null;
    }

    if (filters.entity?.type && filters.entity?.value) {
        const group = entities.value.find((entry) =>
            entry.groupOptions?.some((option) => option.value === filters.entity.value && option.type === filters.entity.type)
        );
        const option = group?.groupOptions?.find((entry) => entry.value === filters.entity.value);
        filterData.value.entity = option || null;
    }
};

const loadEntities = () => {
    if (!props.permissions?.all_cdr_view || !props.routes.entities_route) {
        return;
    }
