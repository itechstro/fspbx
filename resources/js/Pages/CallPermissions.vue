        ...
        <DataTable @search-action="fetchData(1)" @reset-filters="resetFilters">
            ...
            <template #table-header>
                ...
                <TableColumnHeader class="px-2 py-3.5 text-left text-sm font-semibold text-gray-900">
                    <button class="flex items-center" @click="setSort('toll_allow')">
                        <span class="mr-2">Toll Allow</span>
                        <ChevronUpIcon v-if="sortData.name === 'toll_allow' && sortData.order === 'asc'" class="h-4 w-4 text-gray-500" />
                        <ChevronDownIcon v-else-if="sortData.name === 'toll_allow' && sortData.order === 'desc'" class="h-4 w-4 text-gray-500" />
                    </button>
                </TableColumnHeader>
            ...
            <template v-if="selectPageItems" #current-selection>
                <td :colspan="columnCount">
                    <div class="m-2 text-center text-sm">
                        <span class="font-semibold">{{ selectedItems.length }}</span> profiles are selected.
                        <button
                        ...
                        >
                            Select all {{ data.total }} profiles
                        </button>
                        <button
                            v-if="selectAll"
                            class="rounded px-2 py-2 text-blue-500 transition duration-500 ease-in-out hover:bg-blue-200 hover:text-blue-500 focus:bg-blue-200 focus:outline-none focus:ring-1 focus:ring-blue-300"
            ...
            <template #table-body>
                <tr v-for="row in data.data" :key="row.call_permission_uuid">
                    ...
                    <TableField class="whitespace-nowrap px-2 py-2 text-sm text-gray-500">
                        {{ row.toll_allow || "No toll allow" }}
                    </TableField>

                    <TableField class="whitespace-nowrap px-2 py-2 text-sm text-gray-500">
                        {{ row.extensions_count ?? 0 }}
                    </TableField>
...
const confirmAction = (action, items) => {
    const copy = {
        header: "Confirm Copy",
        text: "Copy the selected profile(s)?",
        button: "Copy",
        color: "indigo",
    };
    ...
    const toggle = {
        ...
        text: "Toggle enabled for the selected profile(s)?",
        button: "Toggle",
        color: "indigo",
    };
    ...
    const del = {
        ...
        text: "This action will permanently delete the selected profile(s). Assigned extensions will be unlinked.",
        button: "Delete",
        color: "red",
    };
