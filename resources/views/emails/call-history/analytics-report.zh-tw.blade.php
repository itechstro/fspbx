...
description: 通話紀錄分析電子郵件報告
--}}
<!DOCTYPE html>
<html lang="zh-Hant">
<head>
    <meta charset="UTF-8">
    <title>通話紀錄分析報告</title>
    <style>
...
<div class="container">
    <div class="header">
        <h1 style="margin:0 0 8px;">通話紀錄分析報告</h1>
    ...
    @if(!empty($data['direction_breakdown']))
        <div class="section-title">通話方向</div>
        <div class="sentiment">
    ...
    @if(!empty($data['status_breakdown']))
        <div class="section-title">通話狀態</div>
        <div class="sentiment">
    ...
    @if(!empty($data['recording_status_breakdown']))
        <div class="section-title">錄音可用性</div>
        <div class="sentiment">
    ...
    <div class="section-title">通話</div>
    @if(empty($data['template_calls']))
        <p class="muted">此期間沒有找到通話。</p>
    @else
    ...
    <div class="footer">
        <a href="{{ $data['call_history_url'] ?? '#' }}">開啟通話紀錄</a>
    </div>
</div>
</body>
</html>
